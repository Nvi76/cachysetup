...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --opencode)
      ...
      ;;
    --ollama-opencode)
      TARGET="ollama_opencode"
      ;;
    --ollama-alpaca)
      TARGET="ollama_alpaca"
      ;;
    --alpaca)
      TARGET="alpaca"
      ;;
    ...
    -h|--help)
      ...
      echo ""
      echo "Options:"
      echo "  --ollama           Configure Ollama only (pull models)"
      echo "  --opencode         Configure OpenCode only"
      echo "  --ollama-opencode  Configure Ollama + OpenCode"
      echo "  --ollama-alpaca    Configure Ollama + Alpaca"
      echo "  --alpaca           Show Alpaca configuration only"
...
main() {
  ...
  if [[ "$TARGET" == "alpaca" || "$TARGET" == "ollama_alpaca" || "$TARGET" == "all" ]]; then
    if ! flatpak_app_installed com.jeffser.Alpaca && ! command -v alpaca-ai >/dev/null 2>&1; then
